const COLORS = {
    green: {
        bg: "bg-[#ECF7D4]",
        badge: "bg-[#D6F497]",
    },
    orange: {
        bg: "bg-[#F9EFE1]",
        badge: "bg-[#F7E0B8]",
    },
    red: {
        bg: "bg-[#FBE5E7]",
        badge: "bg-[#FDC6C7]",
    },
};

export const getRandomColor = ()=>{
    const colorNames = Object.keys(COLORS);  // Get array of keys (color names)
    // ["green", "orange", "red"]
        // 0         1       2
    const randomIndex = Math.floor(Math.random() * colorNames.length)   // Get a random index
    // 1
    const randomColorName = colorNames[randomIndex];   // Get the color name at the random index
    // "orange"
    return COLORS[randomColorName]; // Return the color object corresponding to the randosm color name
}